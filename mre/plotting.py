...
def example_images_dice(ds, subj='0219', z=18):
    '''Make a 1x3 grid of images for the important concepts of the analysis'''
    from matplotlib.colors import ListedColormap
    ...
    ds_other = ds.sel(subject=subj, z=z)
    # ds_mri = ds.sel(subject=subj, z=20)
    fig, ax = plt.subplots(1, 3, sharex=False, sharey=False, figsize=(14, 5))
    fig.patch.set_alpha(1)
    # mri = ds_mri.sel(sequence='t1_pre_water')['image_mri']
    # ax[0][0].imshow(mri.T, cmap='gray', vmin=0, vmax=700)
    # ax[0][0].axis('off')
    ...
    combo_mask = ds_other.sel(mask_type='combo')['mask_mre']
    ax[0].imshow(combo_mask.T, cmap=mask_map, interpolation='none', alpha=1)
    ax[0].axis('off')
    ax[0].set_title('ROI (zoomed)', size=18)
    ax[0].set_ylim(170, 45)
    ax[0].set_xlim(30, 170)
    ...
    ax[1].imshow(pred_dice_f1.T, cmap=dice_map_pred, interpolation='none', label='Predicted')
    ax[1].imshow(true_dice_f1.T, cmap=dice_map_true, interpolation='none', label='True')
    ax[1].imshow(true_dice_f1.T*pred_dice_f1.T, cmap=dice_map_inter, interpolation='none',
                 label='Overlap')
    ax[1].axis('off')
    ax[1].set_title('2.88 kPa or Greater', size=18)
    ax[1].set_ylim(170, 45)
    ...
    ax[2].imshow(pred_dice_f4.T, cmap=dice_map_pred, interpolation='none', label='Predicted')
    ax[2].imshow(true_dice_f4.T, cmap=dice_map_true, interpolation='none', label='True')
    ax[2].imshow(true_dice_f4.T*pred_dice_f4.T, cmap=dice_map_inter, interpolation='none',
                 label='Overlap')
    ax[2].axis('off')
    ax[2].set_title('4.09 kPa or Greater', size=18)
    ax[2].set_ylim(170, 45)
    ax[2].set_xlim(30, 170)
    ...
    ax[2].legend(handles=[true_patch, pred_patch, overlap_patch], loc='lower right')
    ax[1].legend(handles=[true_patch, pred_patch, overlap_patch], loc='lower right')
