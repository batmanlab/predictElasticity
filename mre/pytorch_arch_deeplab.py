...
def fixed_padding(inputs, kernel_size, dilation):
    # unclear why this would ne needed
...
class Block(nn.Module):
    def __init__(self, in_channels, planes, reps, stride=1, dilation=1, BatchNorm=None,
                 start_with_relu=True, grow_first=True, is_last=False):
        super(Block, self).__init__()

        if planes != in_channels or stride != 1:
            self.skip = nn.Conv2d(in_channels, planes, 1, stride=stride, bias=False)
        ...
        if grow_first:
            ...
            rep.append(SeparableConv2d(in_channels, planes, 3, 1, dilation, BatchNorm=BatchNorm))
